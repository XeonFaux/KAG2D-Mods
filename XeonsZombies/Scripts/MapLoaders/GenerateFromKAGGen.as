...
bool loadMap( CMap@ _map, const string& in filename)
{
	...
	for(int i = 0; i < ruins_count; i++)
	{
		...
		if(underover <= 0.8)
		{
			...
			{
				if(techstuff_spawn && scrolls_spawn)
				{
					int r = XORRandom(10);
					if(r == 0)
						server_CreateBlob("RocketLauncher",-1,Vec2f((x+_size/2)*8.0,floor_height*8.0 - 16.0));
					else
					if(r == 1)
						server_CreateBlob("megasaw",-1,Vec2f((x+_size/2)*8.0,floor_height*8.0 - 16.0));
					else
					if(r == 2)
						server_MakePredefinedScroll( Vec2f((x+_size/2)*8.0,floor_height*8.0 - 16.0), "carnage" );
					else
					if(r == 3)
						server_MakePredefinedScroll( Vec2f((x+_size/2)*8.0,floor_height*8.0 - 16.0), "midas" );				
					else
					if(r == 4)
						server_MakePredefinedScroll( Vec2f((x+_size/2)*8.0,floor_height*8.0 - 16.0), "tame" );				
					else
				...
				if(!scrolls_spawn)
				{
					...
					if(r == 2)
						server_CreateBlob("superdrill",-1,Vec2f((x+_size/2)*8.0,floor_height*8.0 - 16.0));
					else
						server_CreateBlob("ZombiePortal",-1,Vec2f((x+_size/2)*8.0,floor_height*8.0-24.0));
				}
			}
