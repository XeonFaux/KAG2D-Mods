...
//Modded by Eanmig
//Remodded by 0DarkShadow0
// You may not use my re-mod or scripts for any reason, without permission.
// You may not host my re-mod on a server unless that server is private. (passworded.)
#define SERVER_ONLY
...
void Config(ZombiesCore@ this)
{
    ...
    string configstr = "../Mods/DarkZombies/Rules/DarkZombies/zombies_vars.cfg";
	...
	s32 king_knockback_power = cfg.read_s32("king_knockback_power", 5);
	getRules().set_s32("king_knockback_power", king_knockback_power);

	s32 max_npc_guards = cfg.read_s32("max_npc_guards", 5);
...
shared class ZombiesSpawns : RespawnSystem
{
    ...
    Vec2f getSpawnLocation(PlayerInfo@ p_info)
    {
		...
		if(c_info !is null)
        {
			...
			if(pickSpawn !is null && pickSpawn.getName() == "quarters" && pickSpawn.getTeamNum() == p_info.team)
			{
				CBitStream params;
...
shared class ZombiesCore : RulesCore
{
	    ...
	    if(getGameTime() % (spawnRate) == 0 && num_zombies<100+extra_zombies)
        {
			...
			if(map !is null)
			{
				...
				if(map.getDayTime()>0.8 || map.getDayTime()<0.1)
				{
					...
					if(r==4)
					server_CreateBlob( "monstrosity", -1, sp);
					else
					if(r>=3)
					server_CreateBlob( "zombie", -1, sp);
					else
					if(r==5)
					server_CreateBlob( "chicken", -1, sp);
					else
					if(r==2)
    ...
    void CheckTeamWon( )
    {
		...
		else if(num_zombiePortals == 0)
		{
			rules.SetCurrentState(GAME_OVER);
			rules.SetGlobalMessage("You destroyed all the zombie portals!");
		}
    }
...
void spawnRandomScroll(Vec2f pos)
{
	...
	if(scrolls_spawn)
	{
		...
		if(r<10)
		{
			...
			if(r == 9)
				server_MakePredefinedScroll( pos+Vec2f(0,-16.0), "fish" );
		}			
	}
}
