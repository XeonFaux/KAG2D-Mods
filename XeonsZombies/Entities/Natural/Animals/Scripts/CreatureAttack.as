...
void onInit(CBlob@ this)
{
	...
	if (!this.exists("attack distance"))
	     this.set_f32("attack distance", 8.0f);
...
{
	CMap@ map = this.getMap();
	Vec2f hitBlobPos = hitBlob.getPosition();
	
	if(map.isTileSolid(Vec2f(hitBlobPos.x, hitBlobPos.y)))
	{
		this.server_Hit(hitBlob, hitBlobPos, (hitBlobPos - this.getPosition()), 0.2, Hitters::bite, false);
	}
	if (damage > 0.0f)
	{
