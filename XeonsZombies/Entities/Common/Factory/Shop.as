...
void onCommand( CBlob@ this, u8 cmd, CBitStream @params )
{
	...
	if(cmd == this.getCommandID("shop buy"))
    {
        ...
        if(blobName == "knightbot" || blobName == "archerbot")
        {
	        //Check if max bots for server has been reached
			CBlob@[] bots;
			getBlobsByTag("npc_bot", @bots );
			getRules().set_s32("npc_bots",bots.length());
			int npc_bots = getRules().get_s32("npc_bots");
			int max_npc_guards = getRules().get_s32("max_npc_guards"); 
			if(npc_bots >= max_npc_guards)
			{
				client_AddToChat("Max Guards of "+max_npc_guards+" reached, wait for them to die off to buy more!");
				caller.Chat("Max Guards of "+max_npc_guards+" reached, wait for them to die off to buy more!");
				return;
			}
		}
		if(blobName == "migrant_bot")
        {
			
			//Check if max bots for server has been reached
			CBlob@[] migrant_bots;
			getBlobsByTag("migrant", @migrant_bots );
			//getRules().set_s32("migrant_bot",migrant_bots.length());
			int migrant_alive = migrant_bots.length();
			int max_migrants = getRules().get_s32("max_migrants");
			printf("migrant_alive = "+migrant_alive+"max_migrants = "+max_migrants);
			if(migrant_alive >= max_migrants)
			{
				client_AddToChat("Max migrants of "+max_migrants+" reached, wait for them to die off to buy more!");
				caller.Chat("Max migrants of "+max_migrants+" reached, wait for them to die off to buy more!");
				return;
			}
		}
        ...
        if(inv !is null && isInRadius( this, caller ))
        {
			...
			if(tookReqs && (blobName == "coin_trade"))
			{
				caller.getPlayer().server_setCoins( caller.getPlayer().getCoins() + 125 );
