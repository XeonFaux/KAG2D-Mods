...
void Explode ( CBlob@ this, f32 radius, f32 damage )
{
	...
	bool destroy_dirt = getRules().get_bool("destroy_dirt");
	...
	if(this.isInInventory())
	{
		...
		{
			doomed.server_SetHealth(-1.0f);
			doomed.getSprite().Gib();
			doomed.server_Die();
		}
	}
                            ...
                            if(canHit)
                            {
								...
								if(map.getSectorAtPosition( tpos, "no build") is null)
								{
									...
									if(!map.isTileBedrock(tile))
									{
										...
										if(dist >= rad_thresh || (hitter == Hitters::bomb && map.isTileGroundStuff(tile ))  ) 										{
											if(destroy_dirt || !map.isTileGroundStuff(tile)) map.server_DestroyTile(tpos, 1.0f, this);
										}
										else	  										{
											if(destroy_dirt || !map.isTileGroundStuff(tile)) map.server_DestroyTile(tpos, 100.0f, this);
										}
									}
								}
                            }
...
void LinearExplosion( CBlob@ this, Vec2f _direction, const f32 length, const f32 width,
		...
		for(int width_step = 0; width_step < width_steps + 2; width_step++)
		{
			...
			if(isserver)
			{
				...
				else if(t != CMap::tile_empty && t != CMap::tile_ground_back)
				{
					if(map.getSectorAtPosition( tpos, "no build") is null) 
					{
						if(!map.isTileGroundStuff(t) || destroy_dirt)
...
{					   
	...
	//down
	LinearExplosion( this, Vec2f(0,1), radius, ray_width, steps*0.5f, damage, hitter, blobs, should_teamkill );
	//left and right
