...
const u8 heat_max = 9999999;

const u8 heat_add = 0;
const u8 heat_add_constructed = 0;
...
void onInit( CBlob@ this )
{
	...
	this.getSprite().SetEmitSound( "/Drill.ogg" );	
}
			...
			if(this.getMap().getHitInfosFromArc( (this.getPosition() - attackVel), -attackVel.Angle(), 30, distance, this, false, @hitInfos ))
			{
				...
				for(uint i = 0; i < hitInfos.length; i++)
				{
					...
					if(hi.blob !is null) // blob
					{
						...
						{
							holder.server_Hit( hi.blob, hi.hitpos, attackVel, 1.0f, Hitters::burn ); //extra burn damage
						}
					...
					else // map
					{
						...
						{						
							...
							if( !(getMap().isTileStone( tile ) || getMap().isTileGold( tile )) )
							{
								hitsomething = true;
								if(getMap().isTileCastle(tile) || getMap().isTileWood(tile))
