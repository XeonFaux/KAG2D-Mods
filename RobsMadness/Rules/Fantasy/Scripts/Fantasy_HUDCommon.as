...
shared class WAR_HUD_TEAM
{
	...
	WAR_HUD_TEAM(CTFTeamInfo@ info) //setup what we can from a war team info
	{
		number = info.index;
		halls = 0;
	}
...
shared class WAR_HUD
{
	 ...
	 */
	void Generate(CTFTeamInfo@[] team_structs, CBlob@[] hall_blobs)
	{
		for (uint team_num = 0; team_num < team_structs.length; ++team_num)
		{
			CTFTeamInfo@ team = team_structs[team_num];
