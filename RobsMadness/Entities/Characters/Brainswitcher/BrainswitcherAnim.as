...
{
	const string texname = "Brainswitcher.png"; //These three lines are like coder magic. Don't touch em unless you want it to stop working. Only change the names of the .pngs.
...
void onTick(CSprite@ this)
{
	...
	if (!blob.hasTag(burning_tag)) //Are we burning? If so, lets not screw with the burning animations.
	{
		...
		else if (action2 || (this.isAnimation("actionanim") && !this.isAnimationEnded()))
		{
			this.SetAnimation("actionanim");
		}
		else if (action1  || (this.isAnimation("secondaryanim") && !this.isAnimationEnded()))
		{
	...
	else if (action1 || blob.isInFlames()) //Are we using our left click ability or are we on fire?
	{
