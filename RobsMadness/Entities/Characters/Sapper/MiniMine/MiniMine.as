...
void onDie(CBlob@ this)
{
	if(getNet().isServer() && this.hasTag("exploding"))
	{
		...
		CBlob@[] blobs;
		getMap().getBlobsInRadius(POSITION, this.getRadius() + 15, @blobs);
		for(u16 i = 0; i < blobs.length; i++)
		{
			CBlob@ target = blobs[i];
			if(target.hasTag("flesh") && target.getTeamNum() != this.getTeamNum())
			{
				this.server_Hit(target, POSITION, Vec2f_zero, 2.0f, Hitters::mine_special, true);
			}
		}
	}
}