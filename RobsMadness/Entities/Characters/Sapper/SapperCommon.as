//common sapper header
namespace SapperStates
{
	enum States
	{
		normal = 0,
...
{
	return (state >= SapperStates::sword_drawn && state <= SapperStates::sword_power_super);
}
...
f32 getCutAngle(CBlob@ this, u8 state)
{
	...
	if (state == SapperStates::sword_cut_mid)
	{
		attackAngle += (this.isFacingLeft() ? 30.0f : -30.0f);
	}
	else if (state == SapperStates::sword_cut_mid_down)
	{
		attackAngle -= (this.isFacingLeft() ? 30.0f : -30.0f);
	}
	else if (state == SapperStates::sword_cut_up)
	{
		attackAngle += (this.isFacingLeft() ? 80.0f : -80.0f);
	}
	else if (state == SapperStates::sword_cut_down)
	{
		attackAngle -= (this.isFacingLeft() ? 80.0f : -80.0f);
	}
...
f32 getCutAngle(CBlob@ this)
{
	...
	if (direction == -1)
	{
		tempState = SapperStates::sword_cut_up;
	}
	else if (direction == 0)
	{
		if (aimpos.y < this.getPosition().y)
		{
			tempState = SapperStates::sword_cut_mid;
		}
		else
		{
			tempState = SapperStates::sword_cut_mid_down;
		}
	}
	else
	{
		tempState = SapperStates::sword_cut_down;
	}

	return getCutAngle(this, tempState);
}

//shared attacking/bashing constants (should be in SapperVars but used all over)
