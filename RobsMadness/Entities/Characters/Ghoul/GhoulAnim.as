...
{
	addRunnerTextures(this, "ghoul", "Ghoul");
	
	string texname = "Ghoul.png";
	...
	CSpriteLayer@ shield = this.addSpriteLayer("shield", "FleshShield.png", 28, 28);
	...
	if (shield !is null)
	{
		...
		shield.SetOffset(Vec2f(3,0));
	}
}

void onPlayerInfoChanged(CSprite@ this)
{
	ensureCorrectRunnerTexture(this, "ghoul", "Ghoul");
}
...
void onTick(CSprite@ this)
{
	...
	if(this.getSpriteLayer("shield") !is null){
		...
		else if(Health(blob) > 4.0f){
			frame = 2;
		}
		else if(Health(blob) > 2.0f){
			frame = 1;
		}
		else {
			frame = 0;
		}
		this.getSpriteLayer("shield").SetFrame(frame);
		this.getSpriteLayer("shield").RotateBy(1, Vec2f(0,0));
	}
	...
	else if (ghoul.state == GhoulStates::sword_drawn)
	{
	...
	else if (ghoul.state == GhoulStates::sword_cut_mid_down)
	{
		this.SetAnimation("strike_mid_down");
	}
	else if (ghoul.state == GhoulStates::sword_cut_up)
	{
		this.SetAnimation("strike_up");
	}
	else if (ghoul.state == GhoulStates::sword_cut_down)
	{
	...
	if (shiny !is null)
	{
		...
		if (shinydot)
		{
			f32 range = (GhoulVars::slash_charge_limit - GhoulVars::slash_charge_level2);
			f32 count = (ghoul.swordTimer - GhoulVars::slash_charge_level2);
