...
void onTick(CBlob@ this)
{
	...
	else if ((pressed_a1 || swordState) && !moveVars.wallsliding)   //no attacking during a slide
	{
		         ...
		         ghoul.state <= GhoulStates::sword_cut_down) // cut state
		{
			...
			if (delta > DELTA_BEGIN_ATTACK && delta < DELTA_END_ATTACK)
			{
				...
				DoAttack(this, 0.5f, attackAngle, attackarc, Hitters::sword, delta, ghoul);
			}
			else if (delta >= 9)
			{
				ghoul.swordTimer = 0;
	...
	else if (this.isKeyJustReleased(key_action2) || this.isKeyJustReleased(key_action1) || this.get_u32("ghoul_timer") <= getGameTime())
	{
		ghoul.state = GhoulStates::normal;
	}
...
{
	if(customData == Hitters::suddengib)return damage;
	
	if(damage == 0)return damage;
	
	else if(Health(this) > 6.0f){
		damage *= 1.0f;
	}
	else if(Health(this) > 4.0f){
		damage *= 0.80f;
	}
	else if(Health(this) > 2.0f){
		damage *= 0.60f;
	}
	else {
		damage *= 0.40f;
	...
	//Uncomment this line at the first sign of delta bugs
	//OverHeal(this,-damage);
		...
		for (uint i = 0; i < hitInfos.length; i++)
		{
			...
			if (b !is null && !dontHitMore) // blob
			{
				...
				if (!dontHitMore)
				{
					...
					if(b.hasTag("dead"))final_damage = 0.25; //Ghouls take small bites
					
					this.server_Hit(b, hi.hitpos, velocity, final_damage, type, true);  // server_Hit() is server-side only
					
					if(b.hasTag("dead") && b.get_u8("eaten") < 4 && b.getName() != "fishy")
					if(Health(this) < 8.0f){
						OverHeal(this, 0.25f);
						b.set_u8("eaten",b.get_u8("eaten")+1);
					}
					
					if(b.get_u8("eaten") >= 4){
						this.server_Hit(b, hi.hitpos, velocity, 8, type, true);
					}
