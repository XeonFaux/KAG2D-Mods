...
void onInit(CBlob@ this)
{
	...
	this.addCommandID("weapon_swap");

	this.getCurrentScript().runFlags |= Script::tick_not_attached;
	this.getCurrentScript().removeIfTag = "dead";
	
	this.set_u8("weapon",0);
	//0 - bow
	//1 - knives
...
void ManageGrapple(CBlob@ this, ArcherInfo@ archer)
{
	...
	if (archer.grappling)
	{
		...
		else
		{
			...
			if (archer.grapple_pos.x < map.tilesize ||
			        archer.grapple_pos.x > (map.tilemapwidth - 1)*map.tilesize ||
...
void onTick(CBlob@ this)
{
	...
	if(this.isKeyJustPressed(key_action3))this.SendCommand(this.getCommandID("weapon_swap"));
	
	ManageGrapple(this, archer);

	if(this.get_u8("weapon") == 1)
	...
	if(this.get_u8("weapon") == 0)ManageBow(this, archer, moveVars);
}
...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	if (cmd == this.getCommandID("shoot arrow"))
	{
		...
		if (legolas)
		{
			if (getNet().isServer())
			{
				CBlob @arrow = CreateArrow(this, arrowPos, arrowVel*2, arrowType);
				arrow.Tag("doubledamage");
			}
		...
		else
		{
			if (getNet().isServer())
			{
				CreateArrow(this, arrowPos, arrowVel*1.2, arrowType);
			}
	...
	else if (cmd == this.getCommandID("weapon_swap")){
		if(this.get_u8("weapon") == 0)this.set_u8("weapon",1);
		else if(this.get_u8("weapon") == 1)this.set_u8("weapon",0);
	}
	else
	{
		ArcherInfo@ archer;
...
void onCreateInventoryMenu(CBlob@ this, CBlob@ forBlob, CGridMenu @gridmenu)
{
	...
	CGridMenu@ menu = CreateGridMenu(pos, this, Vec2f(arrowTypeNames.length+1, 2), "Current arrow");
	...
	if (menu !is null)
	{
		...
		AddIconToken("$dagger_icon$", "RangerDagger.png", Vec2f(16, 32), 0);
		CGridButton @button = menu.AddButton("$dagger_icon$", "Switch Weapon", this.getCommandID("weapon_swap"));
	}
}
