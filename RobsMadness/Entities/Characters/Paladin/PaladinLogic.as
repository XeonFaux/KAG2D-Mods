...
void onSetPlayer(CBlob@ this, CPlayer@ player)
{
	if(player !is null)
	{
		player.SetScoreboardVars("ScoreboardIconsMod.png", 6, Vec2f(16, 16));
	}
}
...
void onTick(CBlob@ this)
{
	...
	if(ismyplayer)
	{
		if(!this.isKeyPressed(key_action2))Pickaxe(this);
	}
		...
		if(this.get_s16("timer") > 30){
			...
			if (this.getMap().getBlobsInRadius(this.getPosition(), 128.0f, @blobsInRadius)) 
			{
				for (uint i = 0; i < blobsInRadius.length; i++)
				{
					...
					if(b.getTeamNum() == this.getTeamNum() && !b.hasTag("evil") && !b.hasTag("undead") && !b.hasTag("dead") && b.hasTag("flesh"))
					{
						if(!b.hasTag("Protected"))b.AddScript("BuffProtection.as");
						else b.set_u32("protection_buff",getGameTime()+2);
					}
				}
			}
