...
void onTick(CBlob@ this)
{
	...
	else if ((pressed_a1 || swordState))
	{
		         ...
		         samurai.state <= SamuraiStates::sword_cut_down) // cut state
		{
			...
			if (delta > DELTA_BEGIN_ATTACK && delta < DELTA_END_ATTACK)
			{
				...
				DoAttack(this, 0.5f, attackAngle, attackarc, Hitters::sword, delta, samurai);
			}
			else if (delta >= 9)
			{
				samurai.swordTimer = 0;
	...
	if (myplayer)
	{
		...
		{
			SetHelp(this, "help self action", "samurai", "$Slash$ Slash!    $KEY_HOLD$$LMB$", "", 13);
		}
	}
	...
	else
	{
		...
		if(XORRandom(2) == 0)this.Tag("spin");
		else this.Untag("spin");
	}
}
...
f32 onHit(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitterBlob, u8 customData)
{
	...
	if(hitterBlob.getName() != "samurai"){
		if(damage > 0.75)damage -= 0.5f;
		else if(damage > 0.25)damage = 0.25;
	}
