...
void onTick(CSprite@ this)
{
	...
	SamuraiInfo@ samurai;
	if (!blob.get("samuraiInfo", @samurai))
	{
		return;
	}
	...
	if (samurai.state == SamuraiStates::sword_drawn || samurai.state == SamuraiStates::sword_cut_mid || samurai.state == SamuraiStates::sword_cut_mid_down || samurai.state == SamuraiStates::sword_cut_up || samurai.state == SamuraiStates::sword_cut_down)
	{
		this.SetAnimation("strike_power");

		if (samurai.swordTimer <= 1)
			this.animation.SetFrameIndex(0);

		u8 mintime = 6;
		u8 maxtime = 8;
		if (samurai.swordTimer >= mintime && samurai.swordTimer <= maxtime)
		{
			wantsChopLayer = true;
			chopframe = samurai.swordTimer - mintime;
			chopAngle = -vec.Angle();
		}
	}
	else if (inair)
	...
	if (shiny !is null)
	{
		...
		if (shinydot)
		{
			f32 range = (SamuraiVars::slash_charge_limit - SamuraiVars::slash_charge_level2);
			f32 count = (samurai.swordTimer - SamuraiVars::slash_charge_level2);
...
void onGib(CSprite@ this)
{
	...
	CParticle@ Body     = makeGibParticle("Entities/Characters/Samurai/SamuraiGibs.png", pos, vel + getRandomVelocity(90, hp , 80), 0, 0, Vec2f(16, 16), 2.0f, 20, "/BodyGibFall", team);
	CParticle@ Arm      = makeGibParticle("Entities/Characters/Samurai/SamuraiGibs.png", pos, vel + getRandomVelocity(90, hp - 0.2 , 80), 1, 0, Vec2f(16, 16), 2.0f, 20, "/BodyGibFall", team);
	CParticle@ Shield   = makeGibParticle("Entities/Characters/Samurai/SamuraiGibs.png", pos, vel + getRandomVelocity(90, hp , 80), 2, 0, Vec2f(16, 16), 2.0f, 0, "Sounds/material_drop.ogg", team);
	CParticle@ Sword    = makeGibParticle("Entities/Characters/Samurai/SamuraiGibs.png", pos, vel + getRandomVelocity(90, hp + 1 , 80), 3, 0, Vec2f(16, 16), 2.0f, 0, "Sounds/material_drop.ogg", team);
}
