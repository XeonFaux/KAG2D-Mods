//common samurai header
namespace SamuraiStates
{
	enum States
	{
		normal = 0,
...
{
	return (state >= SamuraiStates::sword_drawn && state <= SamuraiStates::sword_power_super);
}
...
f32 getCutAngle(CBlob@ this, u8 state)
{
	...
	if (state == SamuraiStates::sword_cut_mid)
	{
		attackAngle += (this.isFacingLeft() ? 30.0f : -30.0f);
	}
	else if (state == SamuraiStates::sword_cut_mid_down)
	{
		attackAngle -= (this.isFacingLeft() ? 30.0f : -30.0f);
	}
	else if (state == SamuraiStates::sword_cut_up)
	{
		attackAngle += (this.isFacingLeft() ? 80.0f : -80.0f);
	}
	else if (state == SamuraiStates::sword_cut_down)
	{
		attackAngle -= (this.isFacingLeft() ? 80.0f : -80.0f);
	}
...
f32 getCutAngle(CBlob@ this)
{
	...
	if (direction == -1)
	{
		tempState = SamuraiStates::sword_cut_up;
	}
	else if (direction == 0)
	{
		if (aimpos.y < this.getPosition().y)
		{
			tempState = SamuraiStates::sword_cut_mid;
		}
		else
		{
			tempState = SamuraiStates::sword_cut_mid_down;
		}
	}
	else
	{
		tempState = SamuraiStates::sword_cut_down;
	}

	return getCutAngle(this, tempState);
}

//shared attacking/bashing constants (should be in SamuraiVars but used all over)
