...
bool isBuildableAtPos(CBlob@ this, Vec2f p, TileType buildTile, CBlob @blob, bool &out sameTile)
{
	...
	if (blob !is null)
	if(blob.hasTag("runeblock")){
		
		bool cantBuild = false;
		
		CBlob@[] blobsInRadius;
		if (map.getBlobsInRadius(p, 1.0f, @blobsInRadius))
		{
		...
		if(!cantBuild){
			if ((backtile.type >= CMap::tile_castle_d1 && backtile.type <= CMap::tile_castle_d0) || backtile.type == CMap::tile_castle || backtile.type == CMap::tile_castle_moss){
				return true;
			}
			if ((backtile.type >= CMap::tile_wood_d1 && backtile.type <= CMap::tile_wood_d0) || backtile.type == CMap::tile_wood){
				return true;
			}
		}
	}
