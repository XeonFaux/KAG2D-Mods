...
const f32 arrowMediumSpeed = 20.0f;
const f32 arrowFastSpeed = 40.0f;
//maximum is 15 as of 22/11/12 (see ArcherCommon.as)

const f32 ARROW_PUSH_FORCE = 20.0f;
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1)
{
	if (blob !is null && doesCollideWithBlob(this, blob) && !this.hasTag("collided"))
	{
		...
		if (arrowType == ArrowType::water)
		{
			Vec2f oldVelocity = this.getOldVelocity();
			blob.Tag("force_knock"); //stun on collide
			blob.setVelocity(Vec2f(oldVelocity.x * 4, oldVelocity.y * 4));
			this.server_Die();
			return;
		}
		else
		{
			...
			if (dmg > 0.0f)
			{
				...
				else if(arrowType == ArrowType::bomb)
				{
					if(getNet().isServer())
					{
						for(int i = 0; i < 4+XORRandom(8); i ++)
						{
							CBlob@ smolbomb = server_CreateBlob('bombbutsmall',this.getTeamNum(),this.getPosition());
							if(smolbomb !is null)
							{
								smolbomb.setVelocity(oldVelocity);
							}
						}
					}
				}
				else
					this.server_Hit(blob, point1, initVelocity, 1.5f, Hitters::arrow);
			}
		}
...
void ArrowHitMap(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, u8 customData)
{
	...
	if (arrowType == ArrowType::bomb)
	{
		if (!this.hasTag("dead"))
		{
			if(getNet().isServer())
			{
				int rNum = XORRandom(4);
				for(int i = 0; i < 4+rNum; i ++)
				{
					CBlob@ smolbomb = server_CreateBlob('bombbutsmall',this.getTeamNum(),this.getPosition());
					if(smolbomb !is null)
					{
						smolbomb.setVelocity(Vec2f(XORRandom(0),XORRandom(5)));
					}
				}
			}
			this.Tag("dead");
...
void FireUp(CBlob@ this)
{
	...
	if (map !is null)
	{
		...
		if (!isTeamStructureNear(this))
		{
			...
			map.server_setFireWorldspace(this.getPosition() + Vec2f(0, 1) , true);
			map.server_setFireWorldspace(this.getPosition() + Vec2f(0,-1), true);
			map.server_setFireWorldspace(this.getPosition() + Vec2f(-1,-1), true);
			map.server_setFireWorldspace(this.getPosition() + Vec2f(1,-2), true);
			map.server_setFireWorldspace(this.getPosition() + Vec2f(-1,1), true);
			map.server_setFireWorldspace(this.getPosition() + Vec2f(1,0), true);
			map.server_setFireWorldspace(this.getPosition() + Vec2f(1,1), true);
			map.server_setFireWorldspace(this.getPosition() + Vec2f(1,2), true);
			map.server_setFireWorldspace(this.getPosition() + Vec2f(2,1), true);
		}
	}
}
